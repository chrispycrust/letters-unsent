...
test.describe('Submit page (mocked Guardian)', () => {
  ...
  test('keeps the current Cove message visible in a keyboard-height viewport', async ({ page }) => {
    ...
    const readFocusedMetrics = () => page.evaluate(() => {
      ...
      if (!shell || !guardian || !message || !composer || !sendButton || !expandButton) {
        return null;
      }
    ...
    await expect.poll(async () => {
      ...
      return metrics?.shellBottomDelta ?? Number.POSITIVE_INFINITY;
